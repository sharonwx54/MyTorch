...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        in_size, out_size = input_width, 0
        in_chan = num_input_channels
        for l in range(self.nlayers):
            conv_layer = Conv1d(in_chan, num_channels[l], kernel_sizes[l], strides[l], 
                                conv_weight_init_fn, bias_init_fn)
            self.convolutional_layers.append(conv_layer)
            in_chan = num_channels[l]
            out_size = (in_size-kernel_sizes[l])//strides[l]+1
            in_size = out_size
        
        self.linear_layer = Linear(in_chan*out_size, num_linear_neurons, linear_weight_init_fn, bias_init_fn)
    ...
    def forward(self, A):
        ...
        self.A = A
        for l in range(self.nlayers):
            interZ = self.convolutional_layers[l].forward(self.A)
            self.A = self.activations[l].forward(interZ)
        self.A = self.flatten.forward(self.A)

        # Save output (necessary for error and loss)
        self.Z = self.linear_layer.forward(self.A)
    ...
    def backward(self, labels):
        ...
        # <---------------------
        deriv_y = self.linear_layer.backward(grad)
        deriv_y = self.flatten.backward(deriv_y)
        for l in range(self.nlayers-1, -1, -1):
            deriv = deriv_y*self.activations[l].backward()
            deriv_y = self.convolutional_layers[l].backward(deriv)
