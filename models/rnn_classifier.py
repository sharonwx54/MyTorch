...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for s in range(seq_len):
            x_s  = self.x[:, s, :]#.reshape(batch_size, self.input_size)
            hidden_s = []
            #hidden_prev = hidden
            #hidden = np.zeros((self.num_layers, batch_size, self.hidden_size), dtype=float)
            for l in range(self.num_layers):
                cell = self.rnn[l]
                hidden_l = cell.forward(x_s, self.hiddens[-1][l])
                hidden_s.append(hidden_l)
                x_s = hidden_l

            self.hiddens.append(hidden_s.copy())


        # Get the outputs from the last time step using the linear layer and return it
        logits = self.output_layer.forward(x_s)
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for s in range(seq_len-1, -1, -1):
            for l in range(len(self.rnn)-1, -1, -1):
                h_prev_l = self.hiddens[s+1][l-1] if l != 0 else self.x[:, s, :]
                h_prev_t = self.hiddens[s][l]
                dx, dh_prev = self.rnn[l].backward(dh[l], self.hiddens[s+1][l], h_prev_l, h_prev_t)
                #  * Update dh with the new dh from the backward pass of the rnn cell
                dh[l] = dh_prev
