...
class CTC(object):
	...
	def extend_target_with_blank(self, target):
		...
		skip_connect = []
		for i in range(N):
			if extended_symbols[i] == self.BLANK:
				skip_connect.append(0)
			elif i>1 and (extended_symbols[i] != extended_symbols[i-2]):
	...
	def get_backward_probs(self, logits, extended_symbols, skip_connect):
		...
		# the last two element is fixed 
		beta[-1, -1] = 1
		beta[-1, -2] = 1
		for t in range(T-2, -1, -1):
			beta[t, -1] = beta[t+1, -1]*logits[t+1, extended_symbols[-1]]
			for sym in range(S-2, -1, -1):
				next = beta[t+1, sym+1]*logits[t+1, extended_symbols[sym+1]]
				same = beta[t+1, sym]*logits[t+1, extended_symbols[sym]]
				beta[t, sym] = next+same
	...
	def get_posterior_probs(self, alpha, beta):
		...
		sumgamma = np.zeros((T,))
		gamma = alpha*beta
		sumgamma = np.sum(gamma, axis=1).reshape((-1, 1))
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            target_trun = target[batch_itr, :target_lengths[batch_itr]]
            logits_trun = logits[:input_lengths[batch_itr], batch_itr, :]
            ctc = CTC(self.BLANK)
            extend_sym, skip_connect = ctc.extend_target_with_blank(target_trun)
            forward = ctc.get_forward_probs(logits_trun, extend_sym, skip_connect)
            backward = ctc.get_backward_probs(logits_trun, extend_sym, skip_connect)
            gamma = ctc.get_posterior_probs(forward, backward)
            self.gammas.append(gamma)
            for sym in range(gamma.shape[1]):
                total_loss[batch_itr] -= np.sum(gamma[:, sym]*np.log(logits_trun[:, extend_sym[sym]]))
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            # Process:
            ctc = CTC(self.BLANK)
            target_trun = self.target[batch_itr, :self.target_lengths[batch_itr]]
            logits_trun = self.logits[:self.input_lengths[batch_itr], batch_itr, :]
            extend_sym, skip_connect = ctc.extend_target_with_blank(target_trun)
            #     Truncate the target to target length
            #     Truncate the logits to input length
            #     Extend target sequence with blank
            ...
            for sym in range(gamma.shape[1]):
                dY[:gamma.shape[0], batch_itr, extend_sym[sym]] -= gamma[:, sym]/logits_trun[:, extend_sym[sym]]
